...
def aadhar_input():
    ...
    aadhar_number = st.text_input("Please enter your 12-digit Aadhar Number:")

    if st.button("Submit"):
        if len(aadhar_number) == 12 and aadhar_number.isdigit():
            st.success("Aadhar number validated successfully! Proceeding to the main app...")
            st.session_state['aadhar_valid'] = True
        else:
            st.error("Invalid Aadhar number. Please enter a valid 12-digit Aadhar number.")

if 'aadhar_valid' not in st.session_state:
    st.session_state['aadhar_valid'] = False

if not st.session_state['aadhar_valid']:
    aadhar_input()
else:
    # Main App
    ...
    def main():
        ...
        sessions_df = sessions_df[(sessions_df['timestamp'].dt.date >= date_range[0]) & 
                                  (sessions_df['timestamp'].dt.date <= date_range[1])]
        ...
        # The rest of your tabs and main content here
